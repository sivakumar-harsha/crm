...
class invoice_orc_model extends CI_Model
{
    private $table = "invoice_orc";
    ...
    public function getInvoiceDetails($code,$from_date,$to_date,$policy_class, $policy_gen_from, $invoice_id)
    {
        ...
        $this->db->join("account_tree_orc","acc_commission_ledger_orc.dr_parent_id = account_tree_orc.vchaccid");
    ...
    public function InvoiceRaisedCompanies() {
        ...
        $sql = "Select 
                ...
                where 
                    id in (select insurance_company_id from invoice_orc)";
    ...
    public function getInvoiceByCompanies($company_id) {
        ...
        $sql = "Select 
                    *,
                    (select sum(own_commission) from insurance_voucher_datails_orc where invoice_orc_revision_id = A.invoice_id) as outstand_amt,
                ...
                from
                    (Select 
                        ...
                        (select max(revno) from invoice_orc_revision ir where ir.invoice_orc_id = i.id) as revno,
                        (select max(id) from invoice_orc_revision ir where ir.invoice_orc_id = i.id) as invoice_id  
                    from 
                        invoice_orc i where insurance_company_id = ".$company_id."
                )A";
    ...
    public function getInvoiceByVoucher($voucher_no) {
        ...
        $sql = "Select 
                    revno, invno, revdate 
                from 
                    invoice_orc i, invoice_orc_revision ir 
                where 
                    i.id = ir.invoice_orc_id and 
                    ir.id in (select distinct invoice_orc_revision_id from insurance_voucher_datails_orc where voucher_no = '".$voucher_no."')";
