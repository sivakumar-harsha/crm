...
class invoice_orc_revision_model extends CI_Model
{
    private $table = "invoice_orc_revision";
    ...
    public function getRevisionByInvoice($invoice_id){
        ...
        $sql = "select 
                    *, (select distinct invno from invoice_orc where invoice_orc.id = invoice_orc_revision.invoice_orc_id) as invno
                from 
                    invoice_orc_revision
                where 
                    id in (select id from invoice_orc_revision where invoice_orc_id = '".$invoice_id."') order by id";
    ...
    public function getMaxRevisionByInvoice($invoice_id){
        ...
        $sql = "select 
                ...
                from 
                    invoice_orc_revision
                where 
                    id in (select max(id) from invoice_orc_revision where invoice_Orc_id = '".$invoice_id."')";
    ...
    public function getInvoiceDetails($invoice_rev_id)
    {
        ...
        (select sum(debit)  from acc_commission_ledger_orc where insurer_id = i.insurance_company_id and lead_id = ivd.voucher_no) as debit");   
        
        $this->db->join("invoice_orc i","i.id = ir.invoice_orc_id");
        $this->db->join("insurance_voucher_datails_orc ivd","ivd.invoice_orc_revision_id = ir.id");
        
        $this->db->join('account_tree_orc ac', "ac.insurer_id = i.insurance_company_id");
        $this->db->join('acc_commission_ledger_orc al', "al.dr_parent_id = ac.vchaccid and al.lead_id = ivd.lead_id");
        ...
        return $this->db->get("invoice_orc_revision ir")->result();
    }
}