<!-- Content Wrapper. Contains page content -->
 ...
 <link href="https://cdn.jsdelivr.net/npm/select2@4.0.13/dist/css/select2.min.css" rel="stylesheet" />
     ...
     <div class="row">
            <div class="col-md-3">
                  <h1 style="font-size: 17px;margin-top:0px;">
                         Goods Carrying Vehicle Varient
                  </h1>
            </div>
            <div class="col-md-2">
             ...
             <div class="col-md-2">
                <select class="form-control select2" name="select_gc_brand" id="select_gc_brand">
                </select>
            </div>
             <div class="col-md-2">
                <select class="form-control select2" name="select_gc_model" id="select_gc_model">
                </select>
            </div>
            <div class="col-md-3">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                ...
                <h4 class="modal-title text-center">Create Goods Carrying Vehicle Varient</h4>
            </div>
                  ...
                  <div class='row'>
                    <div class="col-sm-8">
                      <select class="form-control" id="add_fuel_type">
                          ...
                          <?php foreach($fuel as $f) { if($f->id == "1" || $f->id == "2" || $f->id == "5"){?>
  ...
  <script>
    ...
    var s_gc_brand = $("#select_gc_brand").val();
    var s_gc_model = $("#select_gc_model").val();
    ...
    $(document).ready(function(){
      ...
      fetch_varient(s_policy_type,s_gc_brand,s_gc_model);
      check_permission();

      $('#add_brand').change(function(){
          var brand = $('#add_brand').val();
          $.ajax({
            url:"get_gc_model_list",
            data:{brand:brand},
            method:"POST",
      ...
      $("#select_p_type").change(function(){
           ...
           s_gc_brand = $("#select_gc_brand").val();
           s_gc_model = $("#select_gc_model").val();
          fetch_gc_brand(s_policy_type);
         
         fetch_varient(s_policy_type,s_gc_brand,s_gc_model);
      });
      
      $('#select_gc_brand').change(function(){
           s_policy_type = $("#select_p_type").val();
           s_gc_brand = $("#select_gc_brand").val();
           s_gc_model = $("#select_gc_model").val();
          ...
          $.ajax({
            url:"get_gc_model_list",
            data:{brand:s_gc_brand},
            method:"POST",
            success:function(response){
                $("#select_gc_model").html(response);
                $("#select_gc_model").trigger("change");
            },
            error: function(code) {   
                alert(code.statusText);
            },
      ...
      $('#select_gc_model').change(function(){
    ...
    function fetch_varient(s_policy_type,s_gc_brand,s_gc_model)
    {
      var content = "";
      ...
      $("#table_id").DataTable({
          ...
          "ajax":{
            'url':'fetch_gc_varient',
            'method' : "POST",
            'data' : {s_policy_type:s_policy_type,s_gc_brand:s_gc_brand,s_gc_model:s_gc_model},
          }
      });      
    }
    ...
    function fetch_gc_brand(policy_type)
    {
           $.ajax({
                   url : "fetch_brand_by_policy_id",
                   method : "POST",
                   ...
                   success:function(response)
                   {
                       $("#select_gc_brand").html(response);
                       $("#select_gc_brand").trigger("change");
                   }
          });
      }
  
      
    function fetch_model(s_policy_type,s_gc_brand)
    {
      var content = "";
      content += "<div class='table-responsive'>";
      content += "<table id='table_id' class='table table-hover table-bordered'>"; 
      ...
      $("#table_id").DataTable({
          ...
          "ajax":{
            'url':'fetch_gc_model',
            'method' : "POST",
            'data' : {policy_type:s_policy_type,s_gc_brand:s_gc_brand},
          }
      });      
    }
    ...
    function test()
    {
        $.ajax({
                 ...
                 success:function(response)
                 {
                     ...
                     alert(obj["partnerTokenGeneratorInputIO"].sessionId);
                 }
        });
    }
  </script>