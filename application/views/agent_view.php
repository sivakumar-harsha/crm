    ...
    <section class="content-header">
                ...
                <div class="pull-right">
                    <select class="agent_select select2 pull-right" style="margin-top: 10px !important" id="agent_select">
                        ...
                        <?php if($a->id  == $agent_id){ ?>
                            <option value="<?php echo $a->id; ?>" selected><?php echo $a->name." (".$a->agent_pos_code." )"; ?></option>
                        <?php }else{ ?>
                            <option value="<?php echo $a->id; ?>"><?php echo $a->name." (".$a->agent_pos_code." )"; ?></option>
                        <?php } } ?>
                    </select>
                </div>
      ...
      <!--  <div class="nav-tabs-custom" style="margin: 0px;">-->
    	 <!--   <ul class="nav nav-tabs bg-info">-->
		    <!--    <li class="active" id="lead_tab"><a href="#lead_content" data-toggle="tab" aria-expanded="false">Lead</a></li>-->
		    <!--    <li class="" id="graph_tab"><a href="#graph_content" data-toggle="tab" aria-expanded="false">Graph</a></li>-->
		    <!--</ul>-->
      <!--  </div>-->
      </section>
    ...
    <div class="row hidden" id="graph_content" style="margin:10px;">
        <div class="col-md-6">
            <div class="box box-solid">
                ...
                <i class="fa fa-th"></i>
                <h3 class="box-title">Complete Lead Graph</h3>
...
function fetch_all_leads()
{
  ...
  content += "<thead><th>S.No</th><th>Client name</th><th>Mobile Number</th><th>Class</th><th>Policy Type</th><th>Business type</th><th>Area</th><th>Lead Type</th><th>User</th><th>AI</th><th>Due Date</th><th>Action</th></thead>";
  content += "<tbody></tbody>";
  content += "</table>";
  content += "</div>";
...
function fetch_followups()
{
  ...
  $("#table_id").DataTable({
      ...
      "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
      "ajax":{
        'url':'fetch_all_follow_ups_dashboard',
        'method' : "POST",
        'data':{id:id},
      }
  });      
...
function get_agent_details()
{
    $.ajax({
        ...
        success:function(response){
            ...
            load_compare_chart(compare_obj["name"],compare_obj["ot_val"],compare_obj["ag_val"],compare_obj["title"]);
        },
        error:function(code){
